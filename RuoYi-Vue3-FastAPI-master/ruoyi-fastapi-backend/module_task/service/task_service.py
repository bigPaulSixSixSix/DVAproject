...
from module_task.service.validator import TaskValidator
from module_task.service.persistence import TaskPersistence
...
class TaskService:
    ...
    async def process_task_config_services(
        cls, query_db: AsyncSession, raw_data: bytes, current_user_id: int, current_user_name: str
    ) -> CrudResponseModel:
        """
        统一处理任务配置的主流程方法
        包含：结构校验 -> 内容校验 -> 数据持久化
        ...
        # ===== 步骤3：数据持久化 =====
        logger.info('步骤3：开始数据持久化')
        try:
            await TaskPersistence.persist_task_config(query_db, payload, current_user_id, current_user_name)
        ...
        try:
            ...
            return CrudResponseModel(is_success=True, message='任务配置保存成功，但数据查询失败')
