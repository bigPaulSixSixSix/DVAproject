...
function isFirstView() {
  try {
    return selectedTag.value.fullPath === '/workbench' || selectedTag.value.fullPath === visitedViews.value[1].fullPath
  } catch (err) {
    return false
  }
}
...
function toLastView(visitedViews, view) {
  ...
  if (latestView) {
    ...
    // now the default is to redirect to the workbench if there is no tags-view,
    // you can adjust it according to your needs.
    if (view.name === 'Workbench') {
      // to reload workbench page
